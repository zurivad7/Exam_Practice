import { containsNormalizedSubstring, normalizeText } from '../pdf/normalize.js';

const bannedChoiceRegex = /all of the above|none of the above/i;

const sharedKeywordCount = (a, b) => {
  const tokenize = (value) =>
    new Set(normalizeText(value).toLowerCase().split(/[^a-z0-9]+/).filter((token) => token.length > 3));
  const ta = tokenize(a);
  const tb = tokenize(b);
  let hits = 0;
  ta.forEach((token) => {
...
export const runQcChecks = (question, pageTextByNumber, seenQuestions) => {
  ...
  const evidenceMatch = question.evidence.every((evidence) => containsNormalizedSubstring(pageTextByNumber.get(evidence.page) ?? '', evidence.quote));
